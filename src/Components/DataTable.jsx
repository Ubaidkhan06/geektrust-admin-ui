import React, { useEffect, useState } from 'react'
import DataRow from './DataRow'
import Pagination from './Pagination';
import { Box, Button, Table, TableBody, TableCell, TableContainer, TableHead, TableRow } from '@mui/material'
import './main.css'

const DataTable = ({ members, onCheck, onDelete, onDeleteSelected, onEdit }) => {
    const maxItem = 10;
    const [currentPage, setCurrentPage] = useState(1);
    const [allCheckOnPage, setAllCheckOnPage] = useState(false);
    const [anyCheck, setAnyCheck] = useState(false);
    const [token, setToken] = useState(null);
    const [totalPages, setTotalPages] = useState(Math.ceil(members?.length / maxItem));
    const [data, setData] = useState(members.slice((currentPage - 1) * maxItem, (currentPage - 1) * maxItem + maxItem))

    const currentData = () => {
        const begin = (currentPage - 1) * maxItem;
        const end = begin + maxItem;
        setData(members.slice(begin, end))
    }

    useEffect(() => {
        currentData();
    }, [members, currentPage])

    useEffect(() => {
        if (currentPage > totalPages && totalPages !== 0) {
            setCurrentPage(totalPages)
        }
    }, [members?.length, totalPages])

    useEffect(() => {
        setTotalPages(Math.ceil(members?.length / maxItem));
        setAnyCheck(members.reduce((i, member) => i || member.isChecked, false));
    }, [members])

    useEffect(() => {
        setAllCheckOnPage(data.reduce((i, member) => i && member.isChecked, true));
    }, [currentPage, members, data])


    const handleAllRowsCheck = (e) => {
        if (e.target.checked)
            data?.map(row => !row.isChecked && onCheck(row.id));
        else
            data?.map(row => row.isChecked && onCheck(row.id));
    }

    const handleRowCheck = (id) => onCheck(id);

    const handleRowDelete = (id) => onDelete(id);

    const handleDelete = () => onDeleteSelected();

    const handleRowEdit = (row) => onEdit(row);

    const handleEditLock = (id) => setToken(id);

    return (
        <>
            {members?.length > 0 ?
                <div>
                    <TableContainer sx={{padding:'5px'}}>
                        <Table>
                            <TableHead>
                                <TableRow>
                                    <TableCell><input type="checkbox"
                                        onChange={handleAllRowsCheck}
                                        checked={anyCheck ? allCheckOnPage ? "checked" : "" : ""} /></TableCell>
                                    <TableCell><div className="data">Name</div></TableCell>
                                    <TableCell><div className="data">Email</div></TableCell>
                                    <TableCell><div className="data">Role</div></TableCell>
                                    <TableCell><div className="data">Action</div></TableCell>
                                </TableRow>
                            </TableHead>
                            <TableBody>
                                {
                                    data?.map(member =>
                                        <DataRow key={member.id}
                                            row={member}
                                            onRowCheck={handleRowCheck}
                                            onRowDelete={handleRowDelete}
                                            onRowEdit={handleRowEdit}
                                            onClickEdit={handleEditLock}
                                            token={token}
                                        />
                                    )
                                }
                            </TableBody>
                        </Table>
                    </TableContainer>
                    <div>
                        <Button color='secondary' variant='contained' disabled={!anyCheck ? "disabled" : ""} onClick={handleDelete}>Delete seleted</Button>
                        {
                            totalPages > 1 &&
                            <Box display={'flex'} justifyContent='center' >
                                <Pagination
                                    currentPage={currentPage}
                                    dataPerPage={maxItem}
                                    totalData={members.length}
                                    setCurrentPage={setCurrentPage}
                                />
                            </Box>

                        }
                    </div>
                </div>
                : <div>
                    No data found
                </div>
            }
        </>
    )
}

export default DataTable
